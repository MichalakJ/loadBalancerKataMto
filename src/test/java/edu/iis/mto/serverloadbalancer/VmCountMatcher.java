package edu.iis.mto.serverloadbalancer;

import org.hamcrest.Description;
import org.hamcrest.Matcher;
import org.hamcrest.TypeSafeMatcher;

/**
 * Created by Kuba on 2016-06-15.
 */
public class VmCountMatcher extends TypeSafeMatcher <Server> {
    private int expectedVmsCount;

    public VmCountMatcher(int expectedVmsCount) {
        this.expectedVmsCount = expectedVmsCount;
    }

    static Matcher<? super Server> hasVmCountOf(int expectedVmsCount) {
        return new VmCountMatcher(expectedVmsCount);
    }

    protected boolean matchesSafely(Server server) {
        return server.countVms() == expectedVmsCount;
    }

    public void describeTo(Description description) {
        description.appendText("server has vm count of ").appendValue(expectedVmsCount);
    }

    @Override
    protected void describeMismatchSafely(Server item, Description mismatchDescription) {
        mismatchDescription.appendText("server has vm count of ").appendValue(item.countVms());
    }
}
